...
애플리케이션 인터페이스와 온톨로지 스쿨은 기본적으로 한국어로 열립니다. 헤더의
`한국어 / English` 선택기로 언어를 전환할 수 있으며 선택 사항은
`localStorage`에 저장됩니다. 온톨로지 이름, 엔터티 및 관계 이름, 샘플 값,
쿼리 구문, RDF 및 JSON 식별자는 원래 영어 형식으로 유지됩니다.
...
- Node.js 22.22.1 권장(Vite 8에는 최신 Node.js 릴리스 필요)
- npm 9+
...
개발 서버는 한국어 UI로 열립니다. 필요한 경우 헤더의 언어 선택기에서
`English`를 선택하세요. 학습 Markdown을 수정했다면 서버를 시작하기 전에
지역화된 매니페스트를 다시 빌드하세요.
...
영어 학습 콘텐츠는 `content/learn`에서 `public/learn.en.json`으로 컴파일되고,
한국어 콘텐츠는 `docs/ko/content/learn`에서 `public/learn.ko.json`으로
컴파일됩니다. `public/learn.json`은 영어 호환성 매니페스트로 유지됩니다.
